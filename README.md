...
That limit is structural. A plugin command's return value _is_ the reply (`fn(raw_args) -> str | None`), so the handler has no way to hand a request to the agent. Routing `/council` to the bundled skill does not help either: skill slash commands are resolved by scanning the skills directories, and a plugin's bundled skill is not in them — `register_skill()` makes a skill loadable, not slash-addressable.
...
- **Speeches read as speech** — the transcript is what the channel shows, so the protocol asks workers to write the way they would talk, not in clause-numbered prose. The three closing blocks (SUMMARY / FINAL / DECISIONS) stay in document register, because those are deliverables rather than speech.
- **Speak from your own job** — every card tells its worker to argue from its profile's role rather than in generalities, and any panelist without an explicit `roles` entry gets one derived from its `SOUL.md`. Different profiles therefore bring different angles by default, not only when you remember to configure them.
- **Channel relay** — each speaker posts its own speech to the channel under its own identity as the meeting runs; panelists with no messaging credentials are proxied by the moderator with a `▸ <name> — TURN n (대리)` header. Speeches are grouped into one thread where the platform allows it (Slack today; Telegram topics and Discord threads work if you pass the third target segment yourself). **On by default, into the conversation you asked in**: ask for a meeting in a Slack thread and the speeches arrive in that thread — the gateway already knows where the request came from, so you never name a channel. Outside a conversation (CLI, cron) it falls back to whichever channel the moderator can send on. The start output names the destination either way. Meeting content reaches the channel verbatim, so turn it off with `--no-relay` (`relay_off: true` for the tool) when that is not what you want — or set `council: {relay: false}` in a profile that should never broadcast. A target that cannot be posted to (a bot not invited to the channel) is detected by the opening message and disables relay with a warning, rather than losing every speech silently. When the moderator itself cannot send, council relays those speeches instead — from the transcript, against an on-disk record, so nothing is posted twice — but only while `council run` is attached. A relay failure never stops a meeting: it degrades to flat posts, or to no posts at all.
