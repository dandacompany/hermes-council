...
def test_start_exposes_relay_options():
    ...
    # `relay` names a destination and nothing else — see tests/test_relay_off.py
    # for why it must not be able to carry `false`.
    assert props["relay"]["type"] == "string"
    assert props["relay_off"]["type"] == "boolean"
    assert not {"relay", "relay_off"} & set(schemas.START_SCHEMA["parameters"]["required"])
