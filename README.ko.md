...
- **채널 중계** — 각 발언자가 회의가 도는 동안 자기 자격으로 채널에 자기 발언을 올린다. 메신저 자격이 없는 패널은 사회자가 `▸ <이름> — TURN n (대리)` 헤더를 붙여 대신 보낸다. 플랫폼이 허용하면 한 스레드로 묶는다(현재 Slack. 텔레그램 토픽·디스코드 스레드는 대상의 3번째 세그먼트를 직접 주면 된다). **기본은 켜짐** — `--relay`를 주지 않으면 사회자가 보낼 수 있는 채널로 중계되고, 시작 출력이 그 대상을 알려준다. 회의 내용이 채널에 그대로 올라가므로 원치 않으면 `--no-relay`(도구에서는 `relay_off: true`)로 끄거나, 절대 중계하지 않을 프로필에는 `council: {relay: false}`를 넣는다. 보낼 수 없는 대상(봇 미초대 등)은 시작 메시지 전송으로 판별해 경고와 함께 중계를 끈다 — 매 발언마다 조용히 실패하지 않는다. 사회자마저 자격이 없으면 그 발언들은 council이 회의록을 읽어 대리 전송한다(전송 기록을 디스크에 남겨 두 번 보내지 않는다). 단 이 대리 전송은 `council run`이 붙어 있는 동안에만 이뤄진다. 중계 실패는 회의를 멈추지 않고 평면 전송 또는 무전송으로 열화된다.
