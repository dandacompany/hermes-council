...
`/council <요청>`은 번들 스킬을 통해 에이전트에게 닿고, 에이전트가 요청을 읽어 회의를 연다. 플러그인 슬래시 커맨드로는 **일부러 등록하지 않았다** — 플러그인 커맨드는 반환값이 곧 사용자 응답이라 핸들러가 에이전트에게 일을 넘길 수 없고, 실제로 슬랙에서는 지시문이 사용자에게 출력되고 회의가 열리지 않았다.

회의 목록·상태는 `hermes council list` / `hermes council status <slug>` 또는 자연어로 묻는다.
...
- **발언은 말하듯** — 회의록이 그대로 채널에 중계되므로, 프로토콜이 워커에게 조항식 문서투 대신 사람에게 말하듯 쓰라고 지시한다. 다만 SUMMARY·FINAL·DECISIONS 세 블록은 발언이 아니라 산출 문서이므로 지금처럼 간결한 문서체를 유지한다.
...
- **채널 중계** — 각 발언자가 회의가 도는 동안 자기 자격으로 채널에 자기 발언을 올린다. 메신저 자격이 없는 패널은 사회자가 `▸ <이름> — TURN n (대리)` 헤더를 붙여 대신 보낸다. 플랫폼이 허용하면 한 스레드로 묶는다(현재 Slack. 텔레그램 토픽·디스코드 스레드는 대상의 3번째 세그먼트를 직접 주면 된다). **기본은 켜짐, 그리고 요청한 그 대화로 간다** — 슬랙 스레드에서 회의를 열면 발언이 그 스레드에 쌓인다. 게이트웨이가 요청 출처를 알고 있으므로 채널을 적을 필요가 없다. 대화 밖(CLI·cron)에서는 사회자가 보낼 수 있는 채널로 간다. 어느 쪽이든 시작 출력이 대상을 알려준다. 회의 내용이 채널에 그대로 올라가므로 원치 않으면 `--no-relay`(도구에서는 `relay_off: true`)로 끄거나, 절대 중계하지 않을 프로필에는 `council: {relay: false}`를 넣는다. 보낼 수 없는 대상(봇 미초대 등)은 시작 메시지 전송으로 판별해 경고와 함께 중계를 끈다 — 매 발언마다 조용히 실패하지 않는다. 사회자마저 자격이 없으면 그 발언들은 council이 회의록을 읽어 대리 전송한다(전송 기록을 디스크에 남겨 두 번 보내지 않는다). 단 이 대리 전송은 `council run`이 붙어 있는 동안에만 이뤄진다. 중계 실패는 회의를 멈추지 않고 평면 전송 또는 무전송으로 열화된다.
