...
- `roles`를 주지 않으면 각 프로필 `SOUL.md`에서 관점을 뽑아 채운다. 목소리를 더 가르려면 프로필의 SOUL 말투 절을 서로 다르게 쓴다.

## 채널 중계

- **중계는 기본으로 켜져 있고, 요청이 들어온 그 대화(채널·스레드)로 간다.** 사용자가 채널을 말하지 않아도 된다.
- **요청에 중계 얘기가 없으면 `relay`를 아예 넘기지 마라.** 넘기지 않는 것과 끄는 것은 다르다 — 안 넘기면 기본대로 켜진다. 대상을 지정할 때만 `relay="slack:#council"`처럼 준다.
- 끄라는 말이 있을 때만 `relay_off=true`를 넣는다. 발신은 각 프로필이 자기 자격으로 하고, 자격이 없는 프로필의 발언은 사회자가 대리한다.
