...
0) 시작 전 점검: 이번 차례에 해당하는 섹션(같은 프로필·같은 TURN 번호 헤더)이 회의록에 이미 있으면,
   재실행된 카드다 — 이미 한 append와 채널 전송만 건너뛰고, 이번 차례에 남은 나머지 단계는
   그대로 계속 수행하라(다음 카드 생성으로 곧장 건너뛰지 마라 — HITL 재개나 종료 차례라면
   남은 단계는 SUMMARY+FINAL+DECISIONS 작성과 체인 종료일 수 있다).
1) 먼저 {transcript_path} 전문을 읽어 사회자 지시와 이전 발언을 모두 파악한다.
2) 아래 "== 이번 차례 =="의 역할을 수행한다. 발언은 네 프로필의 직무·전문 영역에서 본 것을 말하라 — 누구나 할 수 있는 일반론 대신, 네가 아니면 못 할 지적을 내라.
